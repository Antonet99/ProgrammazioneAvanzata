...
export async function updateWeight(req : any, res : Response){
  ...
  const graph = graph_obj.get("graph");

  let data = requests_b["data"];

  

  if(user["id_user"] === graph["id_creator"]){
    ...
    for(let i in data){
      ...
      data[i]["weight"] = graph[start][end] = Utils.exp_avg(old_weight, new_weight);
